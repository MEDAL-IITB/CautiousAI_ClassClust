...
def tsne_visualize(model,loader, perplexity,learning_rate, n_iter, random_state):
  ...
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/edl_TSNE.png')
################################################################################################################
...
def detect_ood(model, loader,n_clusters, test_type):
  ...
  df_scores.to_csv('/home/abhiraj/DDP/CL/eval/COLORECTAL/edl_test_scores.csv', index=False)
  ...
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/edl_OOD_cdf.png')
  ...
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/edl_OOD_score.png')

  plt.figure()
  plt.plot(FPR_arr, TPR_arr, '-', label='auc')
  plt.title('ROC curve, AUC= '+str(auc_score))
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/edl_AUROC.png')
  ...
  for dat in ood_list:
    ...
    plt.title(str(dat[0])) 
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/COLORECTAL/'+test_type+'_od_edl_'+str(count)+'.png') #####################
    if count == 5:
      break
  ...
  for dat in correct_list:
    ...
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/COLORECTAL/'+test_type+'_edl_'+str(count)+'.png')
    if count == 5:
      break
  ...
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/eval/COLORECTAL/edl_accuracy.png')
############################################################################################
...
def generate_gradcam_vis(model, loader, mean, std):
  ...
  for i, sample in enumerate(loader, 0):
    ...
    for j in range(len(label)):
      ...
      fig.savefig('/home/abhiraj/DDP/CL/eval/COLORECTAL/GRADCAM/EDL/edl_gradcam_'+d_type[j]+'_'+id_classes[target_category]+'_'+bach_class+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'.png')
