...
#od_classes = [classes[2]]                                                                   ######[2,3,5]
id_classes = [classes[6],classes[7],classes[8]]                                                        ######
od_classes = [classes[3],classes[5]]
id_classes.sort()
od_classes.sort()
...
def tsne_visualize(model,loader, perplexity,learning_rate, n_iter, random_state):
  ...
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/softmax_TSNE.png')
...
def detect_ood(model, loader,n_clusters, test_type):
      ...
      for i in range(label.shape[0]):
        if d_type[i] == 'id':
          ...
          if label[i] == prediction[i]:
            ...
            correct_list.append([max_softmax[i].detach().cpu(),1, im_paths[i], og_imgs[i], label[i].detach().cpu()])
          else:
            correct_list.append([max_softmax[i].detach().cpu(),0, im_paths[i], og_imgs[i], label[i].detach().cpu()])
          ...
          od_total += 1
  df_scores.to_csv('/home/abhiraj/DDP/CL/eval/COLORECTAL/softmax_test_scores.csv', index=False)
  ...
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/softmax_OOD_cdf.png')
  ...
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/softmax_OOD_score.png')

  plt.figure()
  plt.plot(FPR_arr, TPR_arr, '-', label='auc')
  plt.title('ROC curve, AUC= '+str(auc_score))
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/softmax_AUROC.png')
  ...
  for dat in ood_list:
    ...
    plt.title(str(dat[0])) 
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/COLORECTAL/'+test_type+'_od_softmax_'+str(count)+'.png') #####################
    if count == 5:
      break
  ...
  for dat in correct_list:
    ...
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/COLORECTAL/'+test_type+'_softmax_'+str(count)+'.png')
    if count == 5:
      break
  ...
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/eval/COLORECTAL/softmax_accuracy.png')
...
def detect_ood_odin(model, loader, temparature, epsilon,n_clusters, test_type):
  ...
  for sample in loader:
    ...
    for i in range(label.shape[0]):
      if d_type[i] == 'id':
        ...
        df_scores = df_scores.append({'type':'od','label':int(label[i]),'pred': 'OOD','score':max_softmax[i].detach().cpu().item(),'path':im_paths[i]}, ignore_index = True )
        od_total += 1
  df_scores.to_csv('/home/abhiraj/DDP/CL/eval/COLORECTAL/odin_test_scores.csv', index=False)
  print(len(id_hist))
  print(len(od_hist))
  max_score = max([max(id_hist),max(od_hist)])
  min_score = min([min(id_hist),min(od_hist)])
  ...
  plt.title('Cummulative density of ID and OD')
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/odin_OOD_cdf.png')
  ...
  plt.title('CLS ACC : '+ str(sum(class_wise_correct)/sum(class_wise_total)) + ' FPR95: '+str(fpr95)+'\n O25: '+str(round(odac25,4))+' O50: '+str(round(odac50,4))+' O75: '+str(round(odac75,4))+' O90: '+str(round(odac90,4))+'\n I25: '+str(round(idac25,4))+' I50: '+str(round(idac50,4))+' I75: '+str(round(idac75,4))+' I90: '+str(round(idac90,4)))
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/odin_OOD_score.png')
  ...
  plt.savefig('/home/abhiraj/DDP/CL/plots/COLORECTAL/odin_AUROC.png')
  ...
  plt.title('Accuracy vs %data left')
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/eval/COLORECTAL/odin_accuracy.png')
...
def generate_gradcam_vis(model, loader, mean, std):
  ...
  for i, sample in enumerate(loader, 0):
    ...
    for j in range(len(label)):
      ...
      fig.savefig('/home/abhiraj/DDP/CL/eval/COLORECTAL/GRADCAM/SOFTMAX/SOFTMAX_gradcam_'+d_type[j]+'_'+id_classes[target_category]+'_'+bach_class+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'.png')
