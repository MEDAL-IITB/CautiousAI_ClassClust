...
tsne_visualize(model,val_loader, 30,200, 1000, 0, 'softmax_ham_before_tsne')
...
for epoch in tqdm(range(1, epoch_num+1)):
    ...
    if acc_val > best_val_acc:
        ...
        print('*****************************************************')
        torch.save(model.state_dict(), "/home/abhiraj/DDP/CL/models/HistoSimCLR/softmax_ham.pt")
        
        
tsne_visualize(model,val_loader, 30,200, 1000, 0, 'softmax_ham_after_tsne')

model.load_state_dict(torch.load("/home/abhiraj/DDP/CL/models/HistoSimCLR/softmax_ham.pt"))
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.savefig('/home/abhiraj/DDP/CL/plots/HAM/softmax_confusion.png')
...
def detect_ood_soft(model, loader,n_clusters, test_type):
      ...
      for i in range(label.shape[0]):
        if d_type[i] == 'id':
          ...
          if label[i] == prediction[i]:
            ...
            correct_list.append([max_softmax[i].detach().cpu().item(),1, im_paths[i], og_imgs[i], label[i].detach().cpu().item()])
          else:
            correct_list.append([max_softmax[i].detach().cpu().item(),0, im_paths[i], og_imgs[i], label[i].detach().cpu().item()])
          ...
          od_total += 1
  df_scores.to_csv('/home/abhiraj/DDP/CL/eval/HAM/softmax_test_scores.csv', index=False)
  print(len(id_hist))
  print(len(od_hist))
  ...
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/HAM/softmax_OOD_cdf.png')
  ...
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/HAM/softmax_OOD_score.png')
  ...
  plt.savefig('/home/abhiraj/DDP/CL/plots/HAM/softmax_AUROC.png')
  ...
  for dat in ood_list:
    ...
    plt.title(str(dat[0])) 
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/HAM/'+test_type+'_od_softmax_'+str(count)+'.png') #####################
    if count == 5:
      break
  ...
  for dat in correct_list:
    ...
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/HAM/'+test_type+'_softmax_'+str(count)+'.png')
    if count == 5:
      break
  ...
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/eval/HAM/softmax_accuracy.png')

detect_ood_soft(model, val_loader, 3, 'no_tf')
...
def detect_ood_odin(model, loader, temparature, epsilon,n_clusters, test_type):
  ...
  for dat in loader:
    ...
    for i in range(label.shape[0]):
      if d_type[i] == 'id':
        ...
        df_scores = df_scores.append({'type':'od','label':int(label[i]),'pred': 'OOD','score':max_softmax[i].detach().cpu().item(),'path':im_paths[i]}, ignore_index = True )
        od_total += 1
  df_scores.to_csv('/home/abhiraj/DDP/CL/eval/HAM/odin_test_scores.csv', index=False)
  print(len(id_hist))
  print(len(od_hist))
  max_score = max([max(id_hist),max(od_hist)])
  min_score = min([min(id_hist),min(od_hist)])
  ...
  plt.title('Cummulative density of ID and OD')
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/HAM/odin_OOD_cdf.png')
  ...
  plt.title('CLS ACC : '+ str(sum(class_wise_correct)/sum(class_wise_total)) + ' FPR95: '+str(fpr95)+'\n O25: '+str(round(odac25,4))+' O50: '+str(round(odac50,4))+' O75: '+str(round(odac75,4))+' O90: '+str(round(odac90,4))+'\n I25: '+str(round(idac25,4))+' I50: '+str(round(idac50,4))+' I75: '+str(round(idac75,4))+' I90: '+str(round(idac90,4)))
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/HAM/odin_OOD_score.png')
  ...
  plt.legend(loc='best')
  plt.title('AUC : '+ str(auc_score))
  plt.savefig('/home/abhiraj/DDP/CL/plots/HAM/odin_AUROC.png')
  ...
  for dat in ood_list:
    ...
    plt.imshow(np.asarray(dat[2]))
    plt.title(str(dat[0])) 
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/HAM/'+test_type+'_od_odin_'+str(count)+'.png') #####################
    if count == 5:
      break
  ...
  for dat in correct_list:
    ...
    plt.imshow(np.asarray(dat[3]))
    plt.title(str(dat[4])+ ' : '+ str(dat[0]))
    plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/HAM/'+test_type+'_odin_'+str(count)+'.png')
    if count == 5:
      break
  ...
  plt.title('Accuracy vs %data left')
  plt.legend(loc='best')
  plt.savefig('/home/abhiraj/DDP/CL/eval/HAM/odin_accuracy.png')
  
detect_ood_odin(model, val_loader,1000,0.002,3,'no_tf')
...
def generate_gradcam_vis(model, loader, mean, std):
  ...
  for i, dat in enumerate(loader, 0):
    ...
    for j in range(len(label)):
      ...
      fig.savefig('/home/abhiraj/DDP/CL/eval/HAM/GRADCAM/SOFTMAX/softmax_gradcam_'+d_type[j]+'_'+str(target_category)+'_'+str(label[j].item())+'_'+file_name+'_'+str(j)+'.png')
