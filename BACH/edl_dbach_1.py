...
def tsne_visualize(model,loader, perplexity,learning_rate, n_iter, random_state):
  ...
  plt.legend(loc = 'best')
  plt.savefig('/home/abhiraj/DDP/CL/plots/DBACH/edl_TSNE.png')
################################################################################################################
...
#     min_val_loss = loss
#     early_stop_counter = 0
#     torch.save(full_model.state_dict(), "/home/abhiraj/DDP/CL/models/HistoSimCLR/edl_dbach.pt")
#   else:
...
####################################################################################################
full_model.load_state_dict(torch.load("/home/abhiraj/DDP/CL/models/HistoSimCLR/edl_dbach.pt"))
...
def detect_ood(model, loader,n_clusters, test_type):
    ...
    for i, sample in enumerate(loader, 0):
      ...
      for i in range(label.shape[0]):
        if d_type[i] == 'id' or d_type[i] == 'od':
          ...
          df_scores = df_scores.append({'type':d_type[i],'label':int(label[i]),'pred': int(prediction[i]),'score':uncertainty[i].detach().cpu().item(),'path':im_paths[i]}, ignore_index = True )
          id_total += 1
        else:
  ...
  df_scores.to_csv('/home/abhiraj/DDP/CL/eval/DBACH/edl_test_scores1.csv', index=False)
  ...
  # plt.legend(loc = 'best')
  # plt.savefig('/home/abhiraj/DDP/CL/plots/DBACH/edl_OOD_cdf.png')
  ...
  # plt.legend(loc='best')
  # plt.savefig('/home/abhiraj/DDP/CL/plots/DBACH/edl_OOD_score.png')

  # plt.figure()
  # plt.plot(FPR_arr, TPR_arr, '-', label='auc')
  # plt.title('ROC curve, AUC= '+ str(auc_score))
  # plt.legend(loc='best')
  # plt.savefig('/home/abhiraj/DDP/CL/plots/DBACH/edl_AUROC.png')
  ...
  #   plt.title(str(dat[0])) 
  #   plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/DBACH/'+test_type+'_od_edl_'+str(count)+'.png') #####################
  #   if count == 5:
  #     break
  ...
  #   plt.savefig('/home/abhiraj/DDP/CL/code/Contrastive/sample_images/DBACH/'+test_type+'_edl_'+str(count)+'.png')
  #   if count == 5:
  #     break
  ...
  # plt.legend(loc='best')
  # plt.savefig('/home/abhiraj/DDP/CL/eval/DBACH/edl_accuracy.png')
############################################################################################
    ...
    for j in range(len(label)):
      ...
      if d_type[j] == 'id':
        if label[j] == prediction[j]:
          save_file_path_he = '/home/abhiraj/DDP/CL/eval/DBACH/GRADCAM/EDL/edl_id_correct_'+bach_class+'_'+id_classes[target_category]+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'_HE.png'
          save_file_path_heat = '/home/abhiraj/DDP/CL/eval/DBACH/GRADCAM/EDL/edl_id_correct_'+bach_class+'_'+id_classes[target_category]+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'_HEAT.png'
        else:
          save_file_path_he = '/home/abhiraj/DDP/CL/eval/DBACH/GRADCAM/EDL/edl_id_incorrect_'+bach_class+'_'+id_classes[target_category]+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'_HE.png'
          save_file_path_heat = '/home/abhiraj/DDP/CL/eval/DBACH/GRADCAM/EDL/edl_id_incorrect_'+bach_class+'_'+id_classes[target_category]+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'_HEAT.png'
      if d_type[j] == 'od':
        save_file_path_he = '/home/abhiraj/DDP/CL/eval/DBACH/GRADCAM/EDL/edl_od_gradcam_'+bach_class+'_'+id_classes[target_category]+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'_HE.png'
        save_file_path_heat = '/home/abhiraj/DDP/CL/eval/DBACH/GRADCAM/EDL/edl_od_gradcam_'+bach_class+'_'+id_classes[target_category]+'_'+im_paths[j].split('/')[-1][:-4]+'_'+str(j)+'_HEAT.png'
