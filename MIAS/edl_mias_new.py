            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if loss_type == 'digamma':
                      loss = edl_digamma_loss(outputs, y.float(), epoch, num_classes, annealing_step, device)
                    elif loss_type == 'log':
                      loss = edl_log_loss(outputs, y.float(), epoch, num_classes, annealing_step, device)
                    else:
                      loss = edl_mse_loss(outputs, y.float(), epoch, num_classes, annealing_step, device)
    ...
    for set in ['test','ood']:
      for inputs, labels in dataloaders[set]:
        ...
        for i in range(len(preds)):
          if set == 'test':
            df_scores = df_scores.append({'im_id':im_id, 'type':'id','label':labels[i].item(), 'pred':preds[i].item(), 'score':uncertainty[i].item()}, ignore_index = True)
          else:
            df_scores = df_scores.append({'im_id':im_id, 'type':'od','label':labels[i].item(), 'pred':preds[i].item(), 'score':uncertainty[i].item()}, ignore_index = True)
          im_id+=1
  df_scores.to_csv('edl_mias_test_scores.csv',index=False)
